...
| G3 | Positive control: generous rank on $W_Q$ alone reaches criterion | ○ | **Gate. Fails → single-matrix rule too strict.** `scripts/run_g3.py` written and unit-tested; it closes the row from G2's artifact without retraining, and refuses (rather than emitting a verdict) on a missing, budget-exceeded, NaN, or mismatched artifact. Blocked only on G2 finishing |
| M1 | QK rank sweep (adapt $W_Q$ only) | ○ | H1. Infrastructure built: `indbw.sweep` (cell config-as-data, run-ID hash, resumability, control gate) + `scripts/run_sweep.py --arm QK`, resumable and shardable across a worker fleet. Blocked on G2/G3 — the runner refuses to start until all four gates have a passing, self-consistent record |
| M2 | OV rank sweep (adapt $W_O$ only) | ○ | H1, H3. Same runner, `--arm OV` |
